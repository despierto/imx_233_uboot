                            ...
                            U8 alignment, U32 blocks_count)
{
    ...
    print_sys("Heap init: addr_%x size_%d alignment_%d blocks_%d", 
        storage_addr, storage_size, alignment, blocks_count);

    if (!blocks_count) {
        print_err("%s", "no block's count");
        return NULL;
    }

    if (!storage_addr) {
        print_err("%s", "storage ptr is null");
        return NULL;
    }

    if (storage_size < SYS_HEAM_MIN_STORAGE_SIZE(alignment, blocks_count)) {
        print_err("small storage size (%d)", 
